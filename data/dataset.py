...
class ResponseSelectionDataset(Dataset):
    ...
    def _annotate_sentence(self, example):
      ...
      if self.hparams.do_eot:
        for utt in example.utterances:
          dialog_context.append(utt + ["[EOT]"])
        
      else:
        for utt in example.utterances:
          dialog_context.append(utt)
      ...
      #print(len(dialog_context), len(segment_ids), len(attention_mask))
    ...
    def _max_len_trim_seq(self, dialog_context, response):
      if len(dialog_context)>self.hparams.max_utterances_turns:
        dialog_context=dialog_context[:self.hparams.max_utterances_turns]
      _every_utterance_len =  self.hparams.max_dialog_len//self.hparams.max_utterances_turns
      new_dialog_context = []
      for _d_context in dialog_context:
            if len(_d_context)>=_every_utterance_len:
                new_dialog_context.extend(_d_context[:_every_utterance_len-1]+["EOT"])
            else:
                new_dialog_context.extend(_d_context+["PAD"]*(_every_utterance_len-len(_d_context)))
     
      while len(response) > self.hparams.max_response_len - 1:
        response.pop() # from the back

      return new_dialog_context, response
